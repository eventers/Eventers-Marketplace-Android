...
class ResellOrSendEventFragment : Fragment(), KodeinAware {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (arguments?.getSerializable("my_event_data") != null)
            myEvent =
                arguments?.getSerializable("my_event_data") as MyEventListResponse.Data
        setUserInfo(myEvent)
    }
    ...
    private fun setUserInfo(myEvent: MyEventListResponse.Data) {
        val userData = AppUtils.getUserPreference(requireContext())
        viewModel.setUserInfo(myEvent, userData, requireContext().deviceId())
    }
